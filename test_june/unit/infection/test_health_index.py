...
def test__smaller_than_one():
    ...
    for i in range(len(index_list.prob_lists[0])):
        index_m = index_list(Person.from_attributes(age=i, sex="m",socioecon_index=0.5))
        index_w = index_list(Person.from_attributes(age=i, sex="f",socioecon_index=0.5))
...
def test__comorbidities_effect():
    ...
    dummy = Person.from_attributes(sex="f", age=60,socioecon_index=0.5)
    feo = Person.from_attributes(sex="f", age=60, comorbidity="feo",socioecon_index=0.5)
    guapo = Person.from_attributes(sex="f", age=60, comorbidity="guapo",socioecon_index=0.5)
...
def test__apply_hospitalisation_correction():
    ...
    dummy = Person.from_attributes(sex="f", age=65,socioecon_index=0.5)
    hi = health_index(dummy)
    adjusted_hi = adjusted_health_index(dummy)
    np.testing.assert_allclose(adjusted_hi, hi)
    
    dummy = Person.from_attributes(sex="f", age=18,socioecon_index=0.5)
    ...
    dummy = Person.from_attributes(sex="f", age=40,socioecon_index=0.5)
    hi = np.diff(health_index(dummy), prepend=0., append=1.)
    adjusted_hi = np.diff(adjusted_health_index(dummy), prepend=0., append=1.)
    assert sum(adjusted_hi) == 1.
